...
- [ ] Produce OCI images for all 7 Spring services via **Spring Boot Cloud Native Buildpacks** (`./mvnw spring-boot:build-image`); no hand-written Dockerfiles (decision #29).
- [ ] Extend `docker-compose.yml` with service entries for `config-service`, `discovery-service`, `gateway`, and the four business services — wired onto a shared internal network with `depends_on` + healthcheck ordering (config → discovery → business → gateway).
- [ ] Per-service env overrides for the `docker` profile: container-DNS URIs (`CONFIG_URI=http://config-service:8888`, `EUREKA_URI=http://discovery-service:8761/eureka`, Postgres/Redis/RabbitMQ/Zipkin hosts), `SPRING_PROFILES_ACTIVE=docker`, JWT secret from `.env`.
- [ ] Keep port exposure internal-only for business services (decision #30); only gateway (8080) + existing platform/UI ports stay mapped to the host.
- [ ] Acceptance: `docker compose up -d --build` brings the full stack healthy, Eureka lists every service, the end-to-end order flow works through the gateway, and `docker compose logs` shows service-tagged JSON.
- [ ] Companion **"Run the full stack in containers"** section in `docs/RUN_AND_TEST.md` covering build-image, compose up, smoke test, and teardown.
...
| **7 — Containerization** | `docker compose up -d --build` brings every service healthy. Eureka (8761) lists all 4 business services + gateway. End-to-end order flow via gateway (8080) works. `docker compose logs gateway` shows service-tagged JSON. |
